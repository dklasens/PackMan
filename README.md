...
- Windows self-updates download the release archive from GitHub and verify it against the published SHA-256 checksum before anything is replaced. The replacement runs in a short-lived helper that only overwrites PackMan's own executable from a staging folder under `%TEMP%\PackMan`, then relaunches the app. Because that helper runs from the executable it is replacing, Windows holds the file open: the old build is renamed to `PackMan.exe.old` beside it and restored if the swap fails, so the install is never left without an executable. The leftover is removed on the next launch, or by the next update's helper when the folder is protected. An elevated helper relaunches PackMan through Explorer so the app comes back at your normal integrity level rather than inheriting the administrator token.
